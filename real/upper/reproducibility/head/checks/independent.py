...
def algebra():
    ...
    assert len(c)==14
    export('exact_algebra.json',{'hermite_closed_form_recurrence_checks':N-1,
        'hermite_derivative_checks':N,'P_degree':Pq.degree(),'P_l1':str(mass),
    ...
    print('EXACT_ALGEBRA: 500 Hermite recurrences, 501 derivative identities; P l1 =',mass,flush=True)
    return c
...
def circle(c,panels):
    ...
    tail=norm/(arb(14)*arb(N)**7).sqrt()
    export('independent_circle.json',{'panels':panels,'order':4,'norm_upper':ball(norm),
           'tail501':ball(tail),'rows':rows,'integral_count':len(S_RECORDS)})
    export('independent_integrals.json',{'records':S_RECORDS})
    print('INDEPENDENT_C4',norm,flush=True)
    print('INDEPENDENT_TAIL501',tail,flush=True)

def head():
    from nodes_exact import certify
    certify()
    ...
    grid=json.loads((ROOT/'run/head_moments_501.json').read_text())
    A={tuple(map(int,k.split(','))):read(v) for k,v in grid['A'].items()}
    assert len(A)==63252
    assert set(A)=={(a,b) for a in range(N+1) for b in range(N+1-a) if (a+b)%2}
    ...
    for a in range(N+1):
        ...
        power=(power*Pq).truncate(N+1)
    coeff=[arb(0) for _ in range(N+1)]
    for (a,b),ab in sorted(A.items(),key=lambda kv:(kv[0][1],kv[0][0])):
        fac=(-1)**b*ab**2
        for d in range(a,min(len(powers[a]),N+1-b)):
            if powers[a][d]: coeff[b+d]+=fac*powers[a][d]
    coeff=[v*arb.pi()/2 for v in coeff]
    for n in range(0,N+1,2): assert coeff[n].is_zero()
    saved=json.loads((ROOT/'run/forward_head_501.json').read_text())
    for n in range(1,N+1,2): assert coeff[n].overlaps(read(saved['coefficients'][str(n)]))
    nonlinear=sum((abs(coeff[n]) for n in range(3,N+1,2)),arb(0))
    export('independent_head.json',{'N':N,'moment_count':len(A),'odd_coefficient_count':len(range(1,N+1,2)),'moments_sha256':sha(ROOT/'run/head_moments_501.json'),'b1':ball(coeff[1]),'nonlinear_head':ball(nonlinear),
        'coefficients':{str(n):ball(coeff[n]) for n in range(1,N+1,2)},
        'all_251_odd_coefficients_agree':True,'all_even_coefficients_exactly_zero':True})
